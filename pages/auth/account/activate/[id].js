...
// grab Token from router, send that to signup endpoint. token contains all info to make a new user
...
const ActivateAccount = ({ router }) => {
    ...
    useEffect(() => {
        let token = router.query.id; // check if we have a token, then grab the name. Makes them avaible in state
        if (token) {
